...
class BERTDataloader():
    def __init__(self, args, dataset):
        ...
        ##
        # self.labeln = args.attlabel
        # self.seqn = args.attseq
        ##
    ...
    def _get_train_dataset(self):
        dataset = BERTTrainDataset(self.train, self.max_len, self.mask_prob, self.max_predictions, 
                    self.sliding_size, self.CLOZE_MASK_TOKEN, self.item_count, self.rng)
        # dataset1 = MYTrainDataset(self.labeln,self.seqn)
        # dataset = ConcatDataset([dataset1,dataset])
        return dataset
    ...
    def _get_eval_dataset(self, mode):
        if mode == 'val':
            dataset = BERTValidDataset(self.train, self.val, self.max_len, self.CLOZE_MASK_TOKEN, 
                        self.val_negative_samples)
        elif mode == 'test':
            dataset = BERTTestDataset(self.train, self.val, self.test, self.max_len, 
...
class BERTTrainDataset(data_utils.Dataset):
    ...
    def __getitem__(self, index):
        ...
        seq = self.all_seqs[index] # allseqs 表示每个用户的所有序列
...
# class MYTrainDataset(data_utils.Dataset):
#     def __init__(self, labeln,seqn):
#         self.seqsdir = pd.read_csv(seqn) # 得到 DataFrame
#         self.seqs = np.array(self.seqsdir)  # 转换为 ndarray [[1], [2], [3]]
#         self.labeldir = pd.read_csv(labeln) 
#         self.labels = np.array(self.labeldir) 
